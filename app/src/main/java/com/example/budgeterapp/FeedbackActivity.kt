...
class FeedbackActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val drawerLayout: DrawerLayout = findViewById(R.id.drawer_layout_feedback)
        ...
        feedback_save.setOnClickListener {
            val myRef = FirebaseDatabase.getInstance().getReference("Feedback")
            val userId=myRef.push().key.toString()
            val rating = feedback_rating.rating
            val obj = FeedbackData(phone, rating, feedback_edtbox.text.toString())
            myRef.child(userId).setValue(obj)
            val intent=Intent(applicationContext,FeedbackActivity::class.java)
            intent.putExtra("key",phone)
            startActivity(intent)
            Toast.makeText(applicationContext,"Your feedback has been recorded!", Toast.LENGTH_SHORT).show()
        }
